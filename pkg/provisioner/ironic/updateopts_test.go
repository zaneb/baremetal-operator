...
func TestGetUpdateOperation(t *testing.T) {
	...
	bar := "bar"
	baz := "baz"
	existingData := map[string]interface{}{"foo": "bar", "foop": &bar_exist}
	cases := []struct {
		Name          string
		Field         string
		NewValue      interface{}
...
func TestTopLevelUpdateOpt(t *testing.T) {
	ops := topLevelUpdateOpt("foo", "bar", "baz", nil)
	assert.Len(t, ops, 1)
	...
	noOps := topLevelUpdateOpt("foo", "bar", "bar", nil)
	assert.Len(t, noOps, 0)
}
...
func TestPropertiesUpdateOpts(t *testing.T) {
	...
	ops := propertiesUpdateOpts(&node, newValues, nil)
	assert.Len(t, ops, 1)
	op := ops[0].(nodes.UpdateOperation)
	assert.Equal(t, nodes.AddOp, op.Op)
...
func TestInstanceInfoUpdateOpts(t *testing.T) {
	...
	ops := instanceInfoUpdateOpts(&node, newValues, nil)
	assert.Len(t, ops, 1)
	op := ops[0].(nodes.UpdateOperation)
	assert.Equal(t, nodes.AddOp, op.Op)
	assert.Equal(t, "quux", op.Value)
